...
router.get("/email/weekly-report", async (req, res): Promise<void> => {
  ...
  const scans = storeFilter
    ? allScans.filter((s) => s.storeLocation.toUpperCase() === storeFilter)
    : allScans;

  if (!scans.length) {
    const msg = storeFilter
      ? `No scans found for store ${storeFilter} in the past ${daysBack} days. No email sent.`
      : `No scans in the past ${daysBack} days. No emails sent.`;
